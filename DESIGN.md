...
- 弹窗内的选项列表使用透明底 `ListTile`（覆盖 `listTileTheme` 的卡片底色），选项之间以 8dp 间距分隔，避免卡片粘连。
...
- Theme 使用 `PredictiveBackPageTransitionsBuilder`（Flutter 的 Android 默认）：返回手势进行中显示 Android 14+ 预测性返回预览，普通跳转回退为 FadeForwards 转场；低版本系统自动回退普通转场。
