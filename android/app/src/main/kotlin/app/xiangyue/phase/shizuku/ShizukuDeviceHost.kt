...
class ShizukuDeviceHost(private val context: Context, private val stopped: (String, String) -> Unit) {
    ...
    companion object {
        fun reason(code: String): String = when (code) {
            ...
            "textConnectionUnavailable" -> "当前设备的虚拟屏文字输入服务不可用"
            "textFocusMissing" -> "虚拟屏上没有已聚焦的输入框"
            "textTargetChanged" -> "虚拟屏输入框或焦点已改变，未继续输入"
            "textSelectionUnavailable" -> "无法确定输入框光标或选区位置，未输入文字"
            "textFieldTooLarge" -> "输入框内容过长，未输入文字"
            "textRequiresManual" -> "此输入框需要用户手动输入"
            "textReadbackMismatch" -> "文字已提交，但输入框内容与预期不一致"
            "textCursorRejected" -> "文字已写入，但输入框拒绝调整光标"
            "textCleanupFailed" -> "文字输入连接未能完整释放"
