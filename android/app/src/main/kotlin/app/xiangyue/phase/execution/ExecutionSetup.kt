...
class ExecutionSetup(private val context: Context, private val files: AppFileDriver,
    ...
    override suspend fun selectFile(directory: Boolean): FileGrant? {
        ...
        try {
            ...
            val uri = withTimeout(120000) { result.await() } ?: return null
    ...
    fun pickerResult(code: Int, data: Intent?) {
        ...
        } catch (_: Exception) { request.completeExceptionally(FlutterError("permissionRequired", "无法保留文件授权", null)) }
    }
