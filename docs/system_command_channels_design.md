...
用户确认：Ubuntu / Termux 在环境设置中全局单选，命令、文件与 Skill 使用会话创建时绑定的环境，已有会话不允许切换；两侧文件独立保留，AI 工具与文件页提供显式复制。Shizuku 独立全局启用，使用其实际授权身份，不再限于 shell UID 2000；Termux 取消应用内使用开关，以系统命令授权为选择门槛，依赖自行管理。独立 su Root 通道、交互 PTY、Termux MCP 双向会话、自动目录同步不在本批。
...
- Shizuku 使用服务返回的实际 UID（包括 shell / root），运行快照记录该身份，派发与 UserService 执行前均核对一致性；不自动提权或切换身份。命令仍为 `/system/bin/sh -c`、默认 `/`。Termux 使用真实应用 UID、固定 Bash、默认 `HOME/.phase/workspaces/<workspaceId>`；不是 Ubuntu `/workspace`，没有路径级强隔离承诺。
...
Shizuku 使用 API/provider 13.1.5 和独立 UserService。Binder 只派发控制，IO 在工作线程，输出使用独立 PFD；服务按客户端 Binder 死亡收尾，并校验调用方仍是相月应用 UID。命令由 APK JNI 目录中的 runner 执行，日志/控制记录放在 `/data/local/tmp/phase-<package>-<uid>`，避免切换 Shizuku 启动身份后复用不同所有者的目录，不把相月私有路径当成通道可读路径。
...
同日按用户要求移除 Shizuku 固定 UID 限制，状态、运行快照与 UserService 改用实际授权身份，仍校验调用方与快照身份一致。应用代码静态检查、改动 Dart 格式及 diff 检查、Profile 构建通过；在设备 `1b8418ca` 以 `adb install -r` 覆盖安装（`Success`）并冷启动（`Status: ok`，1643 ms）。schema 未改，未卸载或清数据；全量检查的既有问题不变，未修改或运行测试，shell/root 实际命令与传输仍待验收。
