...
- 设备执行：提出需求 → 基础档首次应用操作确认 → 连续观察与执行 → 返回实际观察。
...
| 类别 | 计划 | 基础 | 全权限 |
|---|---|---|---|
| 宿主只读：时间、文件读取/列表、历史、Skill/记忆读取、列出应用 | allow | allow | allow |
| 文件写入/编辑、Skill 文件复制 | deny | ask | allow |
| 命令与依赖安装（同一类别，保留两个工具） | deny | ask | allow |
| 控件读取、截图、打开应用、点击、滚动、输入、手势、Shizuku 虚拟屏 | deny | ask（同轮首次批准后复用） | allow |
...
确认绑定 runId、应用侧 toolCallId、固定参数、目标和执行通道，默认 60 秒。展示本次实际内容与批准范围，提供允许、拒绝、停止；关闭或返回不视为批准，也不重置期限。变更参数需新调用。等待期间不占设备动作队列。

基础档的内置 `app_operations` 首次确认提供“允许本轮操作应用”：批准后，同一 `AgentRun` 连续驱动中的后续应用操作共用批准，包括跨模型请求的点击、输入、观察及 Shizuku 虚拟屏操作；文件与命令仍“允许一次”。执行器仅在确认决定成功落库且未停止后持有内存授权，不改写运行模式或工具策略，不为后续调用伪造用户决定。正常结束、失败、停止后失效，中断恢复使用新驱动重新确认，不从历史批准恢复授权。拒绝、超时不授予后续操作权限；每次操作继续核对工具范围、最新撤权、系统授权、名单交集与实际目标。实现入口为 [ToolExecutor](../lib/features/tools/tool_executor.dart)。

支付、密码、验证码由用户手动处理，自动动作不猜目标。Shizuku 虚拟屏无需用户开启应用无障碍服务；2026-09-27 增加的 Unicode 输入以临时节点连接检查当前输入框，不代表其他视觉动作具备完整节点级敏感内容识别，不能宣称保护能力等同。拒绝一个动作后不能通过换工具重复同一效果；停止任务则结束循环。
...
一次发送创建 `AgentRun`，固定连接、模型、参数、上下文窗口、会话权限模式、规划前执行档、记忆范围、助手提示词、工具范围和执行范围；新编辑影响新运行。凭据按配置 ID 在必要时读取，快照不保存密钥。查看其他会话不改变根任务；现阶段同一时间一个根运行。
...
`wait_for_user(prompt)` 用于登录、验证码等必须手动完成的步骤，不用于核验普通动作。运行记录为 `awaitingUser`，该工具保持 executing；同轮后续工具和下一轮模型请求都等待用户点击继续，无自动继续或确认超时。继续绑定 runId / toolCallId，返回后清除旧界面快照，由 AI 重新观察，不宣称操作成功。停止、锁屏、授权丢失或存储故障走正常收尾；重启将等待调用记为中断失败，不重放工具或猜测用户已操作。等待状态仍阻止会话删除、复制及使用中 Skill 清理。
...
默认不按固定轮数截断任务（`AgentRun.maxTurns = 0`）；工具结果回填后继续请求模型，直到模型不再提出工具调用、用户停止或发生终止错误。明确指定正数轮次预算的运行仍按总轮数限制，恢复不会重置预算；耗尽时保存可见的失败说明与已有工具结果，不能只结束运行而让界面看起来正常完成。正常、空回复、停止、轮次耗尽、模型错误与应用记录存储失败有明确终态。
...
当前分支、工具调用与结果成组构建请求；缺失结果补如实的错误信息，跨配置/模型的签名和加密状态不回放。E5.1 使用真实请求负载投影及有效 usage 基准估算下一输入；请求表独立记录每次聊天/摘要尝试，用量按请求、运行、会话汇总，缓存与推理不重复相加。累计摘要只使用上一摘要与新增闭合组，支持单任务长工具循环和每逻辑轮一次超窗恢复；当前任务、最近完整交互、外部动作与失败/拒绝组仍完整保留。旧只读组退出活动窗口后可经有界 `read_history` 找回，历史事实和消耗不删除。独立长期记忆不自动吸收摘要。契约与未验证范围见 [上下文管理与用量方案](./context_management_and_usage_design.md)；本次获授权的 schema 7 → 8 升级将旧用量单独归档，不纳入新请求统计。
...
设备动作共用一条串行队列，确认与用户操作等待均不占队列。Flutter 控制与 Accessibility overlay 回到同一个执行器，原生面板不自行规划。侧边面板只持有有界、节流的最近消息列表，公开思考、正文与工具按顺序原位更新；工具结束到下一次模型输出前显示最近工具结果，不重置为等待响应。任务结束释放执行服务，保留返回与关闭图标；两个入口均销毁本次面板，返回相月后再切到其他 App 不重现。命令在已有无障碍与通知许可且仍在前台时可附带面板，不把无障碍作为 Linux 命令的权限前置。停止、锁屏、授权失效及目标改变结束相应动作链，已有结果保留；显式用户操作等待期间允许用户切换应用，继续后重新检查实际目标与名单。
...
`ProviderProfile.protocol` 显式选择协议，预设填写表单；models.dev 模型元数据按模型 ID 跨服务商匹配，自定义 API 同样生效（仅本地预算，不改变协议请求）；模型列表按 ID 合并，用户显式能力设置优先。密钥只按配置 ID 引用。
