...
| 应用操作 | 允许名单内的应用信息获取、打开应用、无障碍控件树、点击/滚动/输入、任务面板及结果观察 |
| 视觉执行（2026-09-15 增量） | Android 14+ 目标窗口截图、图片回填、截图像素坐标及串行手势组合；沿用应用名单和确认 |
...
- 通知与前台应用感知增强；窗口截图和坐标手势按本次增量前置，契约见第三部分 §4.6。
...
3. **结果可见**：等待确认、执行中、完成、失败和已取消是不同状态。执行后没有人工核验状态。
...
新助手显式填写首版内置工具的默认策略；配置中未列出的工具和空工具范围均为 deny。助手表单展示、模型工具定义与执行器使用同一份已保存策略，编辑草稿不提前影响运行。

应用相关工具统一使用 `app_operations` 策略，覆盖 `list_apps`、`open_app`、`inspect_ui`、`click_node`、`scroll`、`input_text`、`capture_screen`、`perform_gestures`，不允许单工具策略绕过该组。文件、网络和系统信息工具仍保持各自策略。

应用操作不绑定设置中的单个目标 App，而使用应用黑白名单：

- 默认黑名单为空，允许第三方应用；系统应用（含更新后的系统应用）默认禁止，可逐个显式放行。
- 黑名单中显式禁止优先于系统应用放行；白名单模式仅允许白名单中的包名，可以显式允许系统应用。
- 两种模式的名单独立保存；相月自身也出现在应用名单中，和其他第三方应用使用相同权限规则：默认黑名单模式下允许，可显式禁止；白名单模式下仅在选中后允许。不另设自身应用的截图或交互特例，工具三档策略照常生效。
- 用户应用选择器可查看已安装应用并按系统属性筛选、按名称/首次安装时间/安装包大小排序；安装包大小不包含私有数据和缓存，未知大小如实显示。
- 模型仅通过 `list_apps` 获取有效名单允许的应用，过滤在原生执行边界完成并早于分页，返回总数也不包含被禁止项；提示词不传入黑名单包名。
- 打开应用、控件观察、点击、滚动、输入和手势组合携带 `packageName`；`capture_screen` 无参数，由原生在派发时识别当前前台应用，并在结果中返回实际包名。原生核对实际目标、当前安装信息、运行快照名单和最新保存名单；有参数目标时同时核对请求目标与参数包名，不能靠伪造包名或复用其他应用快照绕过限制。
- 名单收紧即时应用；新增允许不扩大正在运行任务的快照范围。进程重启后继续任务仍需与最新保存名单取交集。
...
结果包含已知成功/失败、正文或产物引用、是否截断和必要的错误代码。拒绝是本次调用的结果，不是网络异常。工具错误、超时和不完整响应直接回填，由 AI 根据已有响应或重新读取的现场决定后续步骤，不要求用户填写结果，也不由框架自动重发动作。
...
执行前保存 executing，执行后保存实际响应。外部动作与数据库不能同时提交；动作完成后保存失败时报告存储错误并结束运行，保留已有内容，不声称动作被撤销。没有执行后人工核验状态或表单，不建设自动重放或补偿事务框架。
...
不能把取消请求本身当作外部动作已撤销；取消结束本次请求，已有响应如实保留，不索要人工核验结果。
...
- 未开始的动作由用户选择是否继续。
- 未决确认按原期限显示或结束。
- executing 且没有结果的调用在启动时按中断失败收口，将错误与已有信息交给 AI；不重发该调用。

继续由用户从中断任务页发起，沿用运行创建时的连接、模型能力、参数与工具配置；只派发尚未执行的调用，已保存结果缺少结果消息时仅补回填。不要求用户填写状态；由 AI 用正常的读取工具观察需要了解的现场。停止不把外部效果改写为已撤销。
...
| 模型/网络错误 | 临时错误自动退避并重试当前模型轮；部分输出独立保留，不与新尝试拼接；永久错误或预算耗尽时结束 |
| 工具已知失败 | 回填失败结果，按任务和用户许可决定后续步骤 |
| 工具失败或响应缺失 | 错误及已有响应回填给 AI；框架不自动重发动作 |
| 应用记录存储失败 | 停止发起新动作并说明对话记录未能保存，不能显示“保存成功” |

工具输入文件、临时副本或产物文件的 IO 失败按工具错误回填，不等同于会话/执行记录存储不可用。已知正文与已完成的外部写入仍保留实际结果；预览副本失败不把已完成写入改为未执行。内部副本使用短的应用生成名称，原始文件名仅作为附件元数据与展示名，避免加前缀后超过文件系统长度限制。用户界面展示文件名和实际原因，不展示原始工具 JSON 或 AI 内部恢复指令。

每个逻辑模型轮对明确的临时错误最多自动重试两次（含首次最多三次请求），每次计入 modelAttemptCount，不重复增加 turnCount。网络中断、超时、临时限流、明确的服务端故障和响应中断可以重试，即使已收到部分正文、公开思考或工具参数；鉴权失败、非法请求、上下文超限、额度耗尽和未分类错误不盲目重试。分类只读取状态码及明确错误字段，不匹配错误文案。

重试前保存失败尝试及已收正文/思考；下一次尝试创建同一父节点下的新助手消息，沿用该轮原始请求上下文、模型配置与此前工具结果。失败尝试留在历史分支，不回放其部分输出或协议状态，不继承其用量/计时，不把两份输出拼成一份回复。未完整收口的工具调用不派发。手动重新生成同样保留用户消息之后已有的工具调用与结果，只重做其后的模型回答。

退避默认为 2 秒、4 秒；有效的 Retry-After 作为最早重试间隔。服务端要求超过 60 秒时结束本轮，不截短等待后提前发送。等待期间仍保留停止与运行状态，停止立即撤销等待；新尝试开始前须结束旧请求、排空流式落库并丢弃迟到回调。应用记录存储失败立即停止，不能伪装成可恢复的模型或工具错误。没有普通工具自动重试器，也不通过修改输出上限、推理参数或切换协议消化错误。
...
应用信息查询使用 Android 包管理器，界面观察与动作使用 AccessibilityService；这些工具共用“应用操作”策略：

| 工具 | 输入 | 结果 |
|---|---|---|
| list_apps | 可选名称/包名搜索、排序、分页 | 允许应用的名称、包名、版本、系统属性、首次安装时间、安装包大小、能否打开 |
| open_app | packageName | 打开后的 UiSnapshot 与动作/观察状态 |
| inspect_ui | packageName | UiSnapshot |
| click_node | packageName、snapshotId、nodeId | 动作结果与新观察 |
| scroll | packageName、snapshotId、nodeId、方向 | 动作结果与新观察 |
| input_text | packageName、snapshotId、nodeId、文本 | 输入结果与回读状态 |
...
UiNode
  ...
  focused / checkable / checked / selected
```
...
`open_app` 在策略批准后才启动目标应用，运行宿主的创建不再隐式打开应用。切换目标必须显式指定允许的包名，不能跟随未确认的跨应用跳转。应用列表查询不启动设备前台服务。

### 4.3 点击、滚动与输入

- 对具备标准动作的节点优先调用相应 Accessibility action。
- 控件操作需要手势时使用 dispatchGesture，坐标取已定位节点边界；视觉手势使用 §4.6 的截图坐标契约，不从普通回答文本解析坐标执行。
...
Android 接受动作不等于任务目标完成。执行器分别记录动作回调与之后的观察，不负责让用户判定是否完成：
...
观察使用相关窗口事件和一次新的快照；超时返回明确结果，不无限轮询。固定等待时间不作为唯一成功判据。点击/滚动已被系统接受且取得快照时，即使画面没有变化也返回实际观察（observationChanged=false）；模型据此判断目标是否达成。输入回读不符则直接返回错误和快照，不弹出人工核验。
...
### 4.6 窗口视觉与手势组合（2026-09-15 增量）

- `capture_screen()` 无参数，直接读取手机当前前台页面，不要求模型先查应用列表、传包名或打开应用。原生在派发时识别实际活动应用并校验名单，使用 Android 14+ 的 `AccessibilityService.takeScreenshotOfWindow` 截取该窗口，结果返回实际 `packageName`。不包含其他应用窗口和相月 Accessibility overlay，不用整屏截图降级绕过窗口范围；识别后目标改变则返回错误，不跟随切换到另一应用。受保护窗口、锁屏、需人工处理的界面与截图失败返回明确工具错误。旧系统仍保留控件树功能。
- 截图最长边为 1568px，PNG 最大 4MB。原生返回临时文件和 `screenshotId`、`imageWidth`/`imageHeight`、`screenBounds`、`windowId`、`rotation`、`capturedAt`。Dart 校验 PNG 尺寸，保存为图片产物并清理临时副本；工具记录是图片引用的事实来源。图片在卡片中可打开查看。
- 仅向同时支持图片与工具的模型开放 `capture_screen`；`perform_gestures` 只要求模型支持工具，不依赖图片输入或系统截图能力。当前分支最近一次视觉工具结果的图片在下一轮作为真实图片输入；完整工具结果组之后追加标注来源的观察消息，四协议复用既有图片编码，不把本地路径充当图片 URL，也不拆散多个工具结果的配对。关闭图片能力时由适配器回填省略说明，不读取/发送图片字节。旧截图文件和记录仍保留，不把历次截图全部重复发给模型。
- `perform_gestures(packageName, actions, coordinateSpace?, imageWidth?, imageHeight?)` 接受 1–10 步 `tap`、`double_tap`、`long_press`、`swipe`、`wait`。默认 `coordinateSpace=screen_pixels`，坐标是设备屏幕像素，不是 Flutter dp 或归一化值。使用压缩图片上的坐标时声明 `image_pixels` 及整数 `imageWidth/imageHeight`（1–16384），原生按该坐标平面和当前应用窗口原点/尺寸换算；整组在首次派发前一次换算为屏幕像素，并记录实际坐标。不要求先截图，不传截图 ID。
- 非等待动作需要 `x/y`；滑动另需 `endX/endY`。长按默认 700ms、允许 500–2000ms；滑动/等待默认 400ms、允许 1–2000ms；组合名义时长最多 15000ms。Dart 在确认前校验完整组合，原生再校验全部坐标边界，非法后续步骤不能导致前面的动作先执行。
- 截图是观察数据和坐标参考，不是执行凭证；没有截图有效期、内容事件版本或一次性使用门槛。相同坐标说明可用于新的工具调用；框架仍不重放同一已派发调用。每步核对当前安装信息、最新名单、实际前台应用、动作坐标是否位于当前窗口以及人工处理状态；普通页面刷新、确认面板、动画或流式文字不凭事件计数判定“截图失效”。只组合无需中途重新识别目标的操作；目标不确定时由模型自行重新观察。
- 整组沿用一次固定参数的 allow/ask/deny 决定，同一设备队列内串行执行。系统拒绝、动作异常、超时或停止立即终止后续步骤，返回已结束步骤的回调、实际屏幕坐标、已完成数量和中断时是否已派发。成功后尝试取得真实截图；截图权限/接口不可用、截图异常或图片副本读取失败只记 `observationError`，不把已经完成的手势改成失败，更不会自动重发。应用记录写入失败仍按存储故障结束运行。Android 没有已派发手势的撤销句柄，不宣称取消撤销了外部效果。
- 本批不增加多指手势、输入法/剪贴板替代、键盘按键、全局导航或视觉 OCR 服务；标准文字输入继续使用现有 `input_text`。
...
ChannelError 使用有限的业务分类：permissionRequired、unavailable、targetChanged、invalidArguments、timeout、executionFailed、cancelled。向用户显示实际发生的状态，不维护面向每个网关/ROM 变体的庞大错误编号和自动降级矩阵。
...
- 结果保留实际响应、错误和取消信息，不只返回一个“成功摘要”，也不要求人工核验。
...
子运行的错误与已有响应直接返回父代理，由父代理决定需要哪些只读观察；不要求用户核验。不能因子代理退出或进程被杀就自动重新委派相同动作。
...
RunConfiguration
  ...
  executionScope: { fileUris, appPolicy: { mode, blacklist, whitelist, allowedSystemApps } }
...
复制带工具历史的会话时，运行、调用、结果消息与产物引用一并重映射为独立记录；存在运行中或待处理中断任务时，先处理任务再复制，避免把可执行任务当作历史副本。
...
| ResponseError | 类型化错误 | 结束本次尝试，保留已收到内容；临时错误由 Loop 退避重试 |
...
| 配置不完整 | 表单就地提示，不发送请求 |
| Auth | 提示检查凭据与权限 |
| RateLimit / Quota | 临时限流按有限策略退避重试；明确的额度耗尽提示账户配额问题，不重试 |
| Network / Timeout | 结束当前尝试、保存已收内容，按有限策略重试当前模型轮 |
| InvalidRequest / ContextLimit | 说明参数/上下文问题，不自动修改用户参数 |
| ProviderError | 明确的临时服务端错误可重试；未分类或永久错误直接结束 |
| StorageError | 显示保存失败并结束当前工作 |

网络与存储边界映射为分类错误（`ProviderError.category`），界面展示 `userMessage` 给出的安全文案；协议原文只进诊断，不落库也不上屏。错误解析读取标准字段，不建立面向错误文本变体的正则链或跨协议补救机制。流式响应内的错误事件（`ResponseError`）即本次响应结束：保留已收内容、按分类收口，临时错误进入同一逻辑轮的自动重试；不把网关错误文案当作正文。
