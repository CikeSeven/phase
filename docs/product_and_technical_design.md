...
退避默认为 2 秒、4 秒；有效的 Retry-After 作为最早重试间隔。服务端要求超过 60 秒时结束本轮，不截短等待后提前发送。等待期间仍保留停止与运行状态，停止立即撤销等待；新尝试开始前须结束旧请求、排空流式落库并丢弃迟到回调。存储失败立即停止，不能伪装成可恢复的模型或工具错误。没有普通工具自动重试器，也不通过修改输出上限、推理参数或切换协议消化错误。
