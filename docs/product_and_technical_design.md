...
应用相关工具统一使用 `app_operations` 策略，覆盖 `list_apps`、`open_app`、`inspect_ui`、`click_node`、`scroll`、`input_text`，不允许单工具策略绕过该组。文件、网络和系统信息工具仍保持各自策略。

应用操作不绑定设置中的单个目标 App，而使用应用黑白名单：

- 默认黑名单为空，允许第三方应用；系统应用（含更新后的系统应用）默认禁止，可逐个显式放行。
- 黑名单中显式禁止优先于系统应用放行；白名单模式仅允许白名单中的包名，可以显式允许系统应用。
- 两种模式的名单独立保存；相月自身始终不提供给模型操作，避免模型修改自己的授权配置。
...
- 每次打开、观察、点击、滚动和输入调用必须携带 `packageName`，原生同时核对请求目标、参数包名、当前安装信息、运行快照名单和最新保存名单；不能靠伪造包名或复用其他应用快照绕过限制。
