...
- 取消与存储失败：内置 HTTP 的取消传递到 Dio，覆盖等待响应和正文空闲连接；写请求结果不明确时保留 unknown。文件写入后的存储失败停止循环，不伪装成可继续的普通失败；流式写库串行收尾，防止迟到写入覆盖终态。
- 会话隔离与复制：查看位置不再覆盖根任务状态，其他会话仍可停止并返回运行会话；复制重建运行、工具、结果及产物引用，删除任一副本不影响另一份记录和文件，复制失败回滚并清理新文件。
- 启动恢复：启动核对未完成运行，executing 转 unknown，未决确认保留原期限；中断任务页提供继续、停止、实际结果核验和写入目标只读查看。继续沿用运行快照，仅执行未派发调用或补齐已知结果消息，不重放已执行动作。
