...
`shell(command, cwd?)` 与 `install_packages` 保留两个工具、共用命令权限类别：计划禁止、基础询问、全权限直接执行。命令确认展示工作区、环境和完整命令；shell 不设超时。环境/工作区来自固定运行配置，模型不能通过参数临时切成 Shizuku/Root。内部以 argv 调用选定 shell；MCP 服务则直接启动 executable，不经过 shell 解释。
...
E6 已接入的通道、全局启用与显式递归传输契约见 [系统命令专项设计](./system_command_channels_design.md)。本批未运行测试或真机验收。
...
实现与验证入口见实施计划。当前初版 schema 9 保存会话权限档/返回档与助手 MCP 启用集合；本次明确不做 schema 8 数据转换、不装机。历史安装例外见 AGENTS 第 5 节，旧用量档案不伪造请求，旧升级实现留在 Git 历史。
