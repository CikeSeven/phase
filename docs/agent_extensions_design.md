...
`shell(command, cwd?)` 使用会话创建时绑定的 Ubuntu / Termux；全局选择仅放在环境设置中，只影响新会话，已有会话不可切换。`install_packages` 仅在 Ubuntu 主环境就绪时注入，Termux 依赖自行管理。二者共用命令权限类别：计划禁止、基础询问、全权限直接执行。命令确认展示工作区、环境和完整命令；shell 不设超时。模型不能通过参数临时切换。Shizuku 保留独立工具与全局许可；Termux 不设应用内使用开关，选为主环境须已获系统命令授权，执行仍要求运行组件就绪。MCP stdio 始终使用 Ubuntu 服务专属目录，不随主环境切换。
...
用户依次看到未安装、未运行、未授权、已授权和具体能力状态。UserService 使用 Shizuku 实际授权身份（包括 shell / root），明确 executable/argv、cwd、退出码与输出；运行快照固定实际 UID，身份变化后拒绝旧快照派发，不自动切换或启动独立 su Root 通道。
