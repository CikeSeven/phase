...
2026-09-26 按用户要求收缩为独立虚拟屏设备后端，不再提供普通 shell 或通用文件传输。`shizuku_display` 使用实际授权 UID（包括 shell/root），复用应用操作权限、名单交集、运行快照和设备队列；首期不依赖无障碍。仅操作宿主持有的运行专属屏幕，支持启动应用、截图和视觉输入，停止或运行结束释放资源，不自动回退主屏。

图片经 ImageReader/PFD 与现有 PNG 附件链回填，不创建另一套 Agent。实际前台任务校验不等同节点级敏感页面识别，中文/Emoji 输入、系统覆盖窗口及真实 ROM 支持不凭编译通过宣称可用。完整契约、实现入口与验收边界见 [系统能力专项方案](./system_command_channels_design.md)。
