...
- 动态工具默认不进入助手；用户启用后定义在三档中保持可见，执行由会话模式授权：基础/全权限直接执行，计划禁止 MCP 派发。未知 effectClass 不能在 Plan Mode 当作只读工具。
...
E2 当前实现：上述本地导入、助手范围、版本固定、管理界面和按需文本读取已接通。读取在三档模式均直接允许，助手仅选择 Skill；结果包含内容摘要版本和来源，按 `nextOffset` 继续读取。ZIP/目录的实际边界、四协议闭环与未验收项见实施计划第 4 节；脚本查看不执行脚本；E3 的 `prepare_skill` 复制固定版本，实际执行通过 `shell`。
...
- 重置/卸载是独立用户动作，显示影响；保留会话工作区，工作区随所属会话删除。环境和工作区数据操作不借安装失败自动触发。

### 5.2 工作区与文件

`Workspace`：id、name、environmentId、rootPath、createdAt/updatedAt。发送第一条消息时自动创建并绑定独立工作区，空白聊天页不创建会话或工作区；一次运行固定工作区及可用环境。环境就绪且模型支持工具时首轮自动注入 shell（计划禁止、基础询问、全权限直接执行）；未安装环境仍可使用工作区文件工具。环境变化作用于新运行。

- 每个工作区有自己的持久文件目录，在 Linux 内部映射为 `/workspace`；命令默认在该目录执行，显式 cwd 使用 guest 路径。普通文件工具直接使用工作区相对路径，校验规范化路径和链接目标，避免误操作别的工作区。文件页直接打开根目录；shell 从根目录及子目录收集变化文件，排除附件与 Skill 专用副本目录。
- 导入附件生成工作区副本并记录来源，产物按现有 Attachment 机制登记。SAF 外部文件采用明确导入/导出，不假设 content URI 是可 bind 的文件路径。
- 不提供手动创建、选择、解绑或共享工作区。删除会话前结束活动任务，随后清理全部工作区文件、来源记录、附件、产物和消息；清理失败保留会话与删除标记供重试。复制会话复制独立工作区文件和来源记录，不复制共享 Ubuntu 环境。
...
`shell(command, cwd?)` 使用会话创建时绑定的 Ubuntu / Termux；全局选择仅放在环境设置中，只影响新会话，已有会话不可切换。`install_packages` 仅在 Ubuntu 主环境就绪时注入，Termux 依赖自行管理。二者共用命令权限类别：计划禁止、基础询问、全权限直接执行。命令确认展示工作区、环境和完整命令；shell 不设超时。模型不能通过参数临时切换。Shizuku 只提供独立虚拟屏工具与全局许可，不参与 shell 或文件传输；Termux 不设应用内使用开关，选为主环境须已获系统命令授权，执行仍要求运行组件就绪。MCP stdio 始终使用 Ubuntu 服务专属目录，不随主环境切换。

每次 shell 是独立非交互进程，变量、cd、alias 不跨调用持久；文件变化保留。当前命令不设总时限，依赖停止、输出上限和宿主收尾；连接/控制期限与外部宿主租约不是命令总时限。MCP 工具调用仍有独立期限。

初始模型预览每路最多 64 KiB，普通 stdout/stderr 直接保存在工具结果中；仅某一路超过预览上限时才保存该路的完整日志产物，卡片提供完整日志入口。默认总上限 8 MiB，达到总上限停止进程，记录截断与此前效果。数值集中配置，不能在 Dart/Kotlin 各写一套。大输出不全塞入数据库、平台消息或内存；用户生成的文件仍作为产物收集。
...
E6 后续增加主环境全局选择与新会话固定绑定：命令、文件页、文件工具及 Skill 工作副本共同使用 Ubuntu 或 Termux；聊天输入框不提供环境入口，已有会话不可切换。两侧目录独立保留，workspace_transfer 与文件页显式复制。通道许可、类型化文件桥、复制/删除生命周期及未验收边界见 [系统命令专项设计](./system_command_channels_design.md)。本批未运行测试，安装启动不等于通道功能验收。

### 7.1 Shizuku

2026-09-26 按用户要求收缩为独立虚拟屏设备后端，不再提供普通 shell 或通用文件传输。`shizuku_display` 使用实际授权 UID（包括 shell/root），复用应用操作权限、名单交集、运行快照和设备队列；无需用户启用应用无障碍服务。仅操作宿主持有的运行专属屏幕，支持启动应用、截图和视觉输入，停止或运行结束释放资源，不自动回退主屏。

图片经 ImageReader/PFD 与现有 PNG 附件链回填，不创建另一套 Agent。2026-09-27 增加 Shizuku 临时节点连接的 Unicode 输入，仅编辑虚拟屏目标应用的焦点输入框，无需启用应用无障碍服务或切换输入法。局部输入框检查不等同完整敏感页面识别，中文/Emoji 输入、系统覆盖窗口及真实 ROM 支持仍需分别验收。完整契约、实现入口与验收边界见 [系统能力专项方案](./system_command_channels_design.md)。
...
各通道独立验收安装/授权/撤销/失败和停止。Ubuntu/Termux 验证命令、输出、真实退出及进程回收；Shizuku 验证创建虚拟屏、允许应用、真实截图、定向输入和释放。构建不能替代系统权限、ROM 和 UID 差异验收。仅在用户明确要求时编写或运行测试。
...
同一 AgentLoop 使用会话级 plan/basic/fullAccess 三档。plan 只开放宿主确定的只读工具（应用能力中仅列出应用）；通用 shell 和未确认能力的第三方 MCP 不因描述包含“read”而放行。范围受限的宿主文件读取可以开放。

通过专用 `submit_plan` 工具提交版本化计划，只有宿主收到有效结构才生成计划卡；该工具只保存计划，不执行外部动作。用户可编辑、批准或取消。保存计划正文、来源运行、修订和批准状态；批准针对当前版本，修改后需重新批准。计划来源运行固定进入规划前的基础或全权限档，初始返回档为基础。批准在同一事务恢复该档、记录批准并创建关联的新 AgentRun，以批准计划为输入，重新读取配置和可用范围；执行确认遵循恢复后的档位，不依赖批准时另行选择的模式。UI 提供明确的模式和计划卡，不从正文关键词猜计划已被批准。

### 8.2 上下文预算与摘要

请求编码器与 ContextMeter 共用实际发送负载，预算使用模型窗口、协议有效输出上限及余量；API usage 与本地估算分开。原始工具组永不删除；宿主确认只读的闭合旧组可以被累计摘要整体覆盖，保留状态/参数/产物引用与受范围限制的 `read_history`。当前任务、最近完整交互、图片、未收口组、拒绝/失败及外部动作组仍原样回放，放不下则明确失败。
...
`MemoryEntry` 包含内容、来源消息/运行、所属助手或全局范围、创建/更新时间、启用状态；用户能查看、修改和删除。显式记忆写入在计划档禁止、基础/全权限直接执行，读取在启用的范围内使用；普通工具结果和摘要不会自动升级为长期事实。

起步用文本与关键字检索，返回有来源和长度预算的条目。已删除条目不因自动摘要再次写回。向量索引在规模与检索质量需要时再引入，不作为首次记忆功能依赖。

### 8.4 E5 / E5.1 当前实现边界

- **预算与摘要**：`ContextMeter` 使用协议负载的 UTF-8 保守估算、图片预留和有效 usage 基准。窗口缺省按 models.dev 目录、本地默认 128000 依次解析；输出采用适配器最终参数，未知时按目录输出上限、本地 4096 依次预留，均标来源。80% 阈值自动整理，60% 为目标，按预算保留最近完整组；同模型/推理参数的摘要请求最多三阶段，候选必须有收益且满足预算才激活。单用户长任务可滚动续跑，但不承诺任意文档或外部动作历史都能容纳。
- **摘要来源与计量**：`ContextSummary` 保存新增覆盖、父链、内容指纹、生成状态、采用结果和前后估算；后代共享未修改覆盖可复用，编辑或切离来源则失效。`model_requests` 是 API 用量唯一统计来源，主聊天重试、自动/手动摘要分别归属；复制记录标继承，摘要和锚点不复制。摘要注入历史材料而非系统规则，不写记忆；停止取消底层连接，重启把悬空请求记为 interrupted，包含无 runId 的手动整理。
- **计划**：工具定义与其他模式保持一致，宿主执行白名单只允许时间、文件读取/列出、历史、Skill 读取、已启用记忆读取与应用列表，另有仅保存结构的 `submit_plan`；通用 shell、HTTP、设备动作和 MCP 均在派发前拒绝，不信任只读描述。`submit_plan` 在其他模式拒绝。提交成功后结束计划运行。批准校验最新修订、来源分支与空闲状态，在同一事务写入用户批准消息、关联新运行及批准状态；执行读取最新配置，按规划前执行档确认或直接执行。会话副本重映射计划/运行/消息归属，不复制摘要缓存。
- **记忆**：助手默认关闭，可选择仅助手或助手与全局；`read_memory` 做关键字/文本检索，最多返回 20 条且来源与内容合计不超过 4000 字。`write_memory` 按会话模式授权，助手不设置写入策略；执行时再次检查当前范围。设置中的长期记忆页可添加、编辑、停用、删除；删除后的迟到编辑不会重新创建条目。普通工具结果、摘要及复制会话均不自动生成记忆。

实现与验证入口见实施计划。当前 schema 10 在会话权限档/返回档与助手 MCP 启用集合之上保存主环境、Termux 工作区身份和分环境文件来源；仅支持用户授权的本次 9 → 10 保数据升级，不扩展历史链。安装例外见 AGENTS 第 5 节，旧用量档案不伪造请求。

### 8.5 E5.1：上下文用量与滚动压缩

详细契约统一见 [上下文管理、Token 用量与缓存统计开发方案](./context_management_and_usage_design.md)。建设内容为四协议用量归一化、请求级记录、usage 基准与预算、运行/会话缓存统计，以及 Pi 式滚动摘要和任务内切分。当前占用、累计消耗与缓存命中率分别计算；估算不冒充 API 用量，缓存命中不抵减窗口占用。

计量、统计、滚动检查点和历史读取已接入同一 AgentLoop；未知效果动作采用完整原文保护的保守策略，不只留下模型生成的事实。已完成和未验证检查分开记录于专项方案与实施计划，不授权数据库或设备升级。
...
| 能力 | 进入条件与首个闭环 |
|---|---|
| 独立 su Root 通道 | 已有普通/无障碍/Shizuku 仍不能完成的明确任务；显式 Root 授权、实际身份、一次动作及取消，禁止自动提权切换；不含已授权 Shizuku 自身的 root 身份 |
