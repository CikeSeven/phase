...
更新：2026-09-19｜状态：待实现的目标设计

本文件细化扩展功能，不表示下列模型、页面或接口已经存在。现状与基础契约见 [产品设计](./product_and_technical_design.md)，实施顺序与交付清单见 [实施计划](./implementation_plan.md)。数据结构名称是设计名称，实现时与当前模型统一，不预建空表、空页面或第二套运行框架。
