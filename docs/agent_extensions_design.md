...
更新：2026-09-19｜状态：扩展目标设计；E1 已实现，本机真实 HTTP 闭环通过

本文件细化扩展目标；E1 远程 MCP 已接入，实际支持范围与验证缺口以实施计划第 3 节为准，其余章节仍是待实现设计。现状与基础契约见 [产品设计](./product_and_technical_design.md)，实施顺序与交付清单见 [实施计划](./implementation_plan.md)。数据结构名称是设计名称，实现时与当前模型统一，不预建空表、空页面或第二套运行框架。
...
上下文只常驻已启用 Skill 的 ID、简述；提供 `read_skill(skillId, relativePath?)`，默认读取 SKILL.md，返回版本、来源与实际内容。资源路径限定在该 Skill 目录，超长内容可分段读取。

需要脚本时，将本次启用 Skill 的固定版本资源复制到工作区专用目录并返回 guest 路径；记录安装 ID/版本与副本关系，不把 Android 私有 host 路径交给 guest 执行。安装原件不随脚本修改，工作副本作为工作区文件管理。
...
本节是目标契约，当前 `ExecutionHostApi.execute` 的单次动作 Future 不能直接充当长期双向进程。
