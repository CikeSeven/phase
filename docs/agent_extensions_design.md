...
`shell(command, cwd?)` 使用会话创建时绑定的 Ubuntu / Termux；全局选择仅放在环境设置中，只影响新会话，已有会话不可切换。`install_packages` 仅在 Ubuntu 主环境就绪时注入，Termux 依赖自行管理。二者共用命令权限类别：计划禁止、基础询问、全权限直接执行。命令确认展示工作区、环境和完整命令；shell 不设超时。模型不能通过参数临时切换。Shizuku 保留独立工具与全局许可；MCP stdio 始终使用 Ubuntu 服务专属目录，不随主环境切换。
