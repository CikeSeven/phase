...
- 动态工具默认不进入助手；用户启用后由会话三档模式授权，基础/全权限直接执行，计划不开放 MCP。未知 effectClass 不能在 Plan Mode 当作只读工具。
...
- **计划**：宿主白名单只开放时间、文件读取/列出、历史、Skill 读取、已启用记忆读取与应用列表，另有仅保存结构的 `submit_plan`；通用 shell、HTTP、设备动作和 MCP 均不开放，不信任只读描述。提交成功后结束计划运行。批准校验最新修订、来源分支与空闲状态，在同一事务写入用户批准消息、关联新运行及批准状态；执行读取最新配置，按规划前执行档确认或直接执行。会话副本重映射计划/运行/消息归属，不复制摘要缓存。
