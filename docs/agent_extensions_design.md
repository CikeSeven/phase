...
- **预算与摘要**：`ContextBuilder` 使用 UTF-8 字节的保守文本估算、图片预留和协议余量，不把本地数字当 API usage。窗口未配置时本地采用 32768，输出未配置时预留 4096；模型管理可设置窗口，主请求的模型参数不因摘要改变。80% 阈值开始尝试同模型的独立摘要请求，摘要输出上限为已预留输出与 2048 的较小值，沿用所选推理等级。初版只压缩旧的纯助手叙述；原用户消息、完整工具组、必要产物引用，以及当前任务和上一组完整交互继续原样保留，过大的受保护材料明确失败，不承诺任意长文档/动作历史都能压缩到窗口内。
- **摘要来源**：`ContextSummary` 记录覆盖消息 ID、分支锚点、内容指纹、算法版本与模型来源；编辑覆盖内容或切离来源分支后重新构建。摘要作为有来源的历史数据注入，不进入系统提示词。请求单独落库并记录 API 用量，停止取消底层连接；失败仅在原上下文仍满足预算时继续，不在同一运行对同一来源反复请求。重启恢复未结束运行时将未收口摘要请求记为失败，保留已有材料。
...
实现与验证入口见实施计划 E5；schema 7 的本次 v6 → v7 保数据装机例外见 AGENTS 第 5 节，不扩展其他历史升级链。

### 8.5 E5.1：上下文用量与滚动压缩（待实施）
...
先交付计量与统计，再扩展压缩范围。让闭合历史工具组退出模型窗口时，必须同时交付来源读取、动作事实保护及规范同步；本节不提前取消 §8.4 的现有完整保留策略，不授权数据库或设备升级。
