...
- **预算与摘要**：`ContextMeter` 使用协议负载的 UTF-8 保守估算、图片预留和有效 usage 基准。窗口缺省 32768；输出采用适配器最终参数，未知时本地预留 4096，均标来源。80% 阈值自动整理，60% 为目标，按预算保留最近完整组；同模型/推理参数的摘要请求最多三阶段，候选必须有收益且满足预算才激活。单用户长任务可滚动续跑，但不承诺任意文档或外部动作历史都能容纳。
