...
`Workspace`：id、name、environmentId、rootPath、createdAt/updatedAt。发送第一条消息时自动创建并绑定独立工作区，空白聊天页不创建会话或工作区；一次运行固定工作区及可用环境。环境就绪且模型支持工具时首轮自动注入 shell（默认 ask、尊重显式 deny）；未安装环境仍可使用工作区文件工具。环境变化作用于新运行。
...
- **计划**：宿主白名单只开放时间、文件读取/列出、Skill 读取和已启用记忆读取，另有仅保存结构的 `submit_plan`；通用 shell、HTTP、设备动作和 MCP 均不开放，不信任只读描述。提交成功后结束计划运行。批准校验最新修订、来源分支与空闲状态，在同一事务写入用户批准消息、关联新运行及批准状态；执行读取最新配置，仍逐工具确认。会话副本重映射计划/运行/消息归属，不复制摘要缓存。
- **记忆**：助手默认关闭，可选择仅助手或助手与全局；`read_memory` 做关键字/文本检索，最多返回 20 条且来源与内容合计不超过 4000 字。`write_memory` 默认 ask，可在助手中调整写入策略；执行时再次检查当前范围。设置中的长期记忆页可添加、编辑、停用、删除；删除后的迟到编辑不会重新创建条目。普通工具结果、摘要及复制会话均不自动生成记忆。

实现与验证入口见实施计划 E5 / E5.1；当前初版 schema 8。本次用户明确要求安装后的 v7 → v8 保数据例外见 AGENTS 第 5 节；旧用量只归档，不伪造请求，不扩展其他升级链。
