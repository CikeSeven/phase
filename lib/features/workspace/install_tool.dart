...
class InstallTool extends Tool {
  ...
  String get description =>
      '在 Ubuntu 环境安装常用开发依赖并记录版本：python（python3、pip、venv）、'
      'node（nodejs、npm）或 git-tools（git、ripgrep）。安装经 apt 更新与自愈流程，'
      '可能耗时数分钟；已安装内容跨会话持久保留，重复安装是安全的。';
  @override
  Map<String, dynamic> get inputSchema => {
    ...
    'properties': {
      'profile': {
        'type': 'string',
        'enum': [for (final profile in DependencyProfile.all) profile.id],
        'description': '要安装的依赖组',
      },
    },
    'required': ['profile'],
  ...
  String describeAction(Map<String, dynamic> arguments) {
    final profile = DependencyProfile.byId(
      arguments['profile'] as String? ?? '',
    );
    return profile == null
        ? 'Ubuntu · 安装依赖\n${arguments['profile']}'
        : 'Ubuntu ${workspace?.environmentRevision ?? "24.04 ARM64"} · ${workspace?.name ?? "会话工作区"}\n'
              '安装${profile.label}：${profile.packages.join('、')}';
  }

  @override
  String? validateArguments(Map<String, dynamic> arguments) =>
      DependencyProfile.byId(arguments['profile'] as String? ?? '') == null
      ? '未知的依赖组'
      : null;
  ...
  Future<ToolOutcome> execute(
    ...
    final profile = DependencyProfile.byId(arguments['profile'] as String)!;
    final tail = <String>[];
    try {
      final resolved = installer ?? DependencyInstaller(repository!, driver!);
      final record = await resolved.install(profile, cancellation, (
        step,
        line,
      ) {
        tail.add(
