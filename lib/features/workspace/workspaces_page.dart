    ...
    return AppScaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const Text('下载约 28.5 MiB；基础文件约 99.9 MiB。安装检查通过后可执行命令，工作区文件独立保存。'),
          if (platform.value?.available == false)
            const Text('此设备不支持 Ubuntu 环境，目前仅支持 ARM64。'),
          const SizedBox(height: 8),
          environment.when(
            data: (value) => Text(
              '${_phase(value.phase)}${value.installedBytes > 0 ? ' · ${(value.installedBytes / 1048576).toStringAsFixed(1)} MiB 文件内容' : ''}${value.error == null ? '' : '\n${value.error}'}',
            ),
          ...
          if (operation.busy) ...[
            Text(_phase(operation.phase ?? EnvironmentPhase.checking)),
            const SizedBox(height: 8),
            if (operation.total != null && operation.total! > 0)
              LinearProgressIndicator(value: operation.bytes / operation.total!)
            else
            ...
            Wrap(
              ...
              children: [
                FilledButton(
                  onPressed:
                      environment.hasValue && platform.value?.available == true
                      ? () async {
