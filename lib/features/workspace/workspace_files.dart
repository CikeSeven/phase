...
String workspacePrompt(WorkspaceSnapshot? workspace) => workspace == null
    ? ''
    : '\n\n本次工作区：${jsonEncode(workspace.name)}；Ubuntu ${workspace.environmentRevision}。guest 路径 /workspace，命令默认在此执行；每次 shell 的变量与 cd 不保留。将要返回的文件写入 /workspace/output，调用后会保存为会话产物。其他文件留在工作区；本会话附件在执行前复制到 /workspace/imports/<附件ID>/<文件名>。Skill 脚本先用 prepare_skill 复制固定版本，再通过 shell 显式调用解释器；没有的依赖如实返回缺失，不自动安装。';
