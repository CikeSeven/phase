...
- 上下文计量、请求/Token/缓存统计与滚动压缩的当前契约和验收边界见 [专项开发方案](./docs/context_management_and_usage_design.md)；自动化通过不等于真实摘要质量或真机验收，也不扩大设备数据操作授权。
...
- `ProviderProfile.protocol` 明确选择协议，预设用于填写创建表单；模型元数据按模型 ID 跨服务商匹配，精确 ID 优先于命名空间末段，同级优先已选预设并稳定排序；目录只参与本地预算，不根据域名、模型名或异常自动切换协议。模型获取与手动管理按 ID 合并，显式能力设置优先。
...
- 文件工具统一使用 `path`：文件名原样保留，不补扩展名；`write_file` 创建或完整覆盖并允许空内容，`edit_file` 按原文唯一匹配后整体替换，`read_file` 从第 1 行分页并返回续读位置。SAF 同样遵循这些语义，覆盖确认由现有工具策略控制，不要求 AI 传覆盖开关或校验值。
...
- 2026-09-22 用户在 E5 实现后要求提交、推送并装机，授权本次 schema 6 → 7 增量覆盖升级：仅新增上下文摘要、计划修订、记忆表和助手记忆范围列，保留设备数据并验证；不扩展其他开发期兼容链。
- 2026-09-22 用户在 E5.1 后再次明确要求安装，授权本次 schema 7 → 8 保数据覆盖升级：新增请求表与旧用量档案表、同步摘要检查点结构；旧 usage 原样归档、不伪造成请求记录、不参与新统计，其他业务记录、配置、凭据和文件保留。仅验证这一次升级，不扩展其他历史兼容链。
- 2026-09-24 会话权限模式调整采用 schema 9 初版契约，最初不转换 schema 8。随后用户要求安装并修复旧库无法加载，授权本次 8 → 9 保数据例外：已有会话默认基础档，助手已开放 MCP 转为启用集合、Skill/记忆范围保留；旧运行 execute 转 basic、plan 保留，规划前档位补 basic，其他运行快照及业务数据、凭据和文件不变。事务失败回滚，仅支持这一次升级，不扩展历史安装链。
...
- 普通运行默认不设固定轮次上限，工具结果回填后继续交给模型；停止、终止错误与权限检查保持有效。显式轮次预算耗尽必须保存可见原因，不得静默停在工具卡片后或伪装为任务完成。
- 上下文预算与 API 用量分开标识；API 累计只来自请求表，每次真实尝试独立计量，缺失不当零。摘要不删除历史：闭合、宿主确认只读的旧交互组可整体退出模型窗口，保留来源并用 `read_history` 有界读取；保留区间不能出现孤立调用/结果。当前任务、最近完整交互、未收口调用、拒绝/失败及外部动作组继续保留原文；未知效果 MCP、shell 和设备动作不靠自然语言摘要淘汰。摘要失败不无限重试，预算无法容纳时保存明确失败。
