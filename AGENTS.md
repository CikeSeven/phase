...
- Ubuntu/Termux 只在环境设置中全局选择，默认 Ubuntu；新会话首次发送时绑定当时已保存的主环境，创建后不可切换，修改全局设置不影响已有会话。聊天输入框不放环境入口或环境草稿；shell、普通文件工具、文件页和 Skill 工作副本使用会话固定后端。两侧会话目录独立保留，workspace_transfer 与文件页显式复制，不自动同步。Shizuku 保持独立全局启用；Termux 仍须显式开放并完成系统授权，不托管其依赖。运行固定集合与身份，新增授权不扩大旧快照，撤权停止活动调用。删除会话清理两侧托管目录，失败保留重试；任意外部导出目标不随会话删除。schema 10 仅支持第 5 节已授权的本次升级，不扩展历史链。实现及未验收边界见 `docs/system_command_channels_design.md`。
