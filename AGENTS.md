...
- 文件工具统一使用 `path`，本地相对路径以当前会话独立工作区为根目录；文件名原样保留，不补扩展名。`write_file` 创建或完整覆盖并允许空内容，`edit_file` 按原文唯一匹配后整体替换，`read_file` 从第 1 行分页并返回续读位置。SAF 同样遵循这些语义，覆盖确认由现有工具策略控制，不要求 AI 传覆盖开关或校验值。
...
- 2026-09-25 主环境切换实现后，用户确认装机，授权仅本次 9 → 10 保数据覆盖升级：已有会话默认 Ubuntu，新增 Termux 工作区身份可空列，文件来源表增加 Ubuntu 环境并调整复合主键；原有来源内容、其他业务数据、运行快照、凭据和文件保留。事务失败回滚，不扩展其他历史升级链。
...
- 前端面向用户，只保留必要的字段、状态、操作和错误提示；未经用户明确要求，不添加解释实现、数据来源、统计口径或使用方法的注释性文案，不在面板底部堆叠说明。工程解释写入文档，不自行放进产品界面。
...
- 用户已授权后续应用代码改动完成、完成必要检查且构建成功后默认安装 Profile 包并启动，无需再次询问；纯文档改动除外。每次先确认已授权设备，目标不明确、设备不可用或安装失败时如实说明；本授权不扩大数据操作或 schema 升级范围，不包含测试、提交和推送。
...
- 权限按会话独立保存为计划、基础、全权限三档，新会话默认基础；不绑定助手。2026-09-26 用户允许三档保留相同工具定义，但执行权限不变：计划仅允许宿主只读白名单及内部计划提交，应用工具中仅 `list_apps` 属只读；基础档对写文件/编辑/Skill 复制、命令（含依赖安装）及其他应用操作 ask，其余已开放工具 allow；全权限档 allow。`submit_plan` 定义常驻，但仅计划档可执行。工具目录按已启用范围、能力和环境固定排序，不因 deny 移除定义；禁止调用必须在执行器派发前拒绝，不启动通道或连接。系统授权、应用名单、能力和扩展启用范围不被模式绕过，运行中不切模式。计划批准绑定当前修订，恢复来源运行记录的规划前执行档并原子创建关联运行；记忆仍限启用范围，摘要和普通工具结果不自动写入长期记忆。
- 2026-09-27 用户要求基础档应用操作按一整轮连续运行确认：首次内置 `app_operations` 调用获准后，同一运行驱动内的后续应用操作（含 Shizuku 虚拟屏）不再逐次询问；批准仅保存在该驱动内存，结束、停止或中断恢复后不继承，新一轮重新确认。拒绝、过期或停止不产生授权；文件与命令仍逐次确认。每次调用仍核对固定范围、最新撤权、系统授权、应用名单与实际目标，逐次保存结果，不伪造新的用户批准记录。
- 固定提示词不拼入会话模式等动态状态；权限、环境和 Skill 状态按分区变化追加 `RuntimeContextPart` 系统消息，文本与分支/运行位置同事务持久化，不改写旧前缀。空闲预览只投影，重试不重建输入，恢复/复制按当前分支比较状态；摘要保留每个分区最新状态原文。OpenAI 使用原位 system/developer，Anthropic/Google 以带宿主标记的 user 文本留在原位，不猜测历史内 system 扩展；提示词不替代执行器授权。详见上下文专项方案。
- 动态工具按稳定来源 ID 和定义修订注册；MCP/Skills 仍由助手选择启用范围，但不保存独立执行策略。服务器说明、Skill 指导和插件清单不授予权限；停用、移出范围和系统撤权即时生效，新增许可不扩大旧快照。MCP 定义在启用范围内保持可见，计划档禁止执行，基础/全权限档直接执行。
- 发送第一条消息时创建会话专属工作区；空白聊天页不落库、不显示文件入口。不提供手动新建、选择或共享工作区；复制会话复制独立文件，删除会话清理工作区、附件、产物与来源记录，失败保留可重试状态。所选主环境就绪且模型支持工具时，按会话模式注入 shell；install_packages 仅在 Ubuntu 主环境注入，两者共用命令权限类别，计划档 deny、基础档 ask、全权限档 allow。
- Ubuntu 安装成功后自动继续安装完整开发依赖（python3/pip/venv、nodejs/npm、git/ripgrep），不再要求单独安装；依赖失败或取消保留 Ubuntu 和已安装内容，仅依赖缺失或未安装完整时提供“修复环境”继续相同的依赖安装流程，不重新下载或替换 rootfs。全部完成后详情不显示修复入口；重开页面或应用仍按持久化依赖记录显示未完成状态。Termux 依赖仍由用户自行管理。
- Ubuntu/Termux 只在环境设置中全局选择，默认 Ubuntu；新会话首次发送时绑定当时已保存的主环境，创建后不可切换，修改全局设置不影响已有会话。聊天输入框不放环境入口或环境草稿；shell、普通文件工具、文件页和 Skill 工作副本使用会话固定后端。两侧会话目录独立保留，workspace_transfer 与文件页显式复制，不自动同步。Shizuku 虚拟屏保持独立全局开关，须服务运行且系统授权成功后才能开启，保存前重新核对，授权失效时仍可关闭；2026-09-26 用户要求移除 shell UID 2000 限制，使用 Shizuku 实际授权 UID（包括 root），运行快照和 UserService 仍须核对身份一致，不自动提权或切换。Termux 不再设置应用内使用开关，未获系统命令授权时禁选 Termux，选择保存前再次核对授权。Termux 执行仍须系统授权、外部调用配置和运行组件就绪，不托管其依赖；切回 Ubuntu 不撤销已有 Termux 会话的可用性。运行固定集合与身份，新增授权不扩大旧快照，撤权停止活动调用。删除会话清理两侧托管目录，失败保留重试；任意外部导出目标不随会话删除。schema 10 仅支持第 5 节已授权的本次升级，不扩展历史链。实现及未验收边界见 `docs/system_command_channels_design.md`。
- 2026-09-26 用户要求收缩 Shizuku 为设备后端，并明确首期无需无障碍独立运行。移除 Shizuku 通用 shell 与文件传输，仅按已启用范围和模型视觉能力开放 `shizuku_display`；它归应用操作权限，复用运行快照、实际 UID、应用名单和串行队列，不接受任意 displayId，不自动回退主屏。首期虚拟屏随运行释放，截图沿现有附件链回填；不宣称拥有无障碍节点级密码/支付识别，不绕过必须人工处理的步骤。Ubuntu/Termux 仍是命令、文件和 Skill 后端。实现及未验收范围见系统命令专项方案。
- 2026-09-27 用户要求虚拟屏支持中文。文字输入在已授权 Shizuku 内以临时 UiAutomation 节点连接直接编辑 Unicode，仅定位本运行虚拟屏目标应用的焦点输入框，保持光标插入/选区替换语义；不要求启用应用无障碍服务、不切换输入法、不使用剪贴板、不改输主屏。不支持的控件、连接失败及回读不符明确返回，不重发已派发输入；密码/验证码字段的局部检查不等于完整敏感页面识别。详见系统命令专项方案。
- Ubuntu 命令使用 PRoot；Termux 主环境使用自身私有目录和原生 runner 文件桥，不把远端路径当成本地 dart:io 文件。MCP stdio 仍固定 Ubuntu 服务专属目录和独立 stdin/stdout/stderr 管道，不随主环境切换。PTY 只用于交互终端。进程按运行/调用归属，取消和超时回收受管理子进程与 FD，重启不重放命令。
