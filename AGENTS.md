...
- `ProviderProfile.protocol` 明确选择协议，预设用于填写创建表单及映射模型元数据目录；目录只参与本地预算，不根据域名、模型名或异常自动切换协议。模型获取与手动管理按 ID 合并，显式能力设置优先。
...
- Plan Mode 以宿主只读范围限制派发，计划批准绑定当前修订并创建关联执行运行，不替代工具确认。记忆仅在启用的助手/全局范围检索，显式写入默认 ask；摘要和普通工具结果不自动写入长期记忆。
